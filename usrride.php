...
<header>
      <nav  class="navbar navbar-expand-lg">
          <a class="navbar-brand nos" href="#">Go<span class="gree">Cheeta</span></a>
    ...
    if($_GET['action']=='blk')
        {
            ...
            $adm = new user();
            $admc = new dbcon();
            $sho = $adm->ridec($ap,$id,$admc->connect());
        }
}
include('header.php');
...
<div class="mr-2" id="srt">
  ...
  <option value="GoCheetaMini">GoCheetaMini</option>
  <option value="GoCheetaMicro">GoCheetaMicro</option>
  <option value="GoCheetaRoyal">GoCheetaRoyal</option>
  <option value="GoCheetaSUV">GoCheetaSUV</option>
  </select>
  </div>
</div>
  ...
  <h3 class="text-center">All Rides</h3>
    ...
    <table id="tbl" class="container-fluid col-lg-10 mr-lg-2 table table-responsive table-hover table-bordered table-striped">
        ...
        <?php 
            ...
            $adm = new user();
            $admc = new dbcon();
            $showr = $adm->allride($id,$admc->connect());
    ...
    <?php 
        ...
        $adm = new user();
        $admc = new dbcon();
        $en = $adm->earn($id,$admc->connect());
        ?>
    <h1 class="text-center font-weight-bold text-dark">LKR<?php echo $en; ?></h1>
    </div>
